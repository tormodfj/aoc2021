...
let rec stepUntilAllFlashing n =
    step()
    let allFlashing = octos |> Array.forall (fun o -> o.Energy = 0)
    if allFlashing then n else stepUntilAllFlashing (n+1)

stepUntilAllFlashing 1
|> printfn "%i"
